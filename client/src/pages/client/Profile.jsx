...
import {
  Sparkles, Home as HomeIcon, Mic, BookOpen, Languages, Settings,
  Check, Flame, Award, Trophy, X, Upload
} from 'lucide-react';
import Header from '../../components/Header';
import { getCurrentUser, getLeaderboard, getLatestLesson, getAttendanceStreak } from '../../services/user.service';
...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setCreateDate(userRes.data.createDate )

        const leaderboardRes = await getLeaderboard();
        setLeaderboard(leaderboardRes.data);

        const lessonRes = await getLatestLesson();
        setLatestLesson(lessonRes.data);

        const streakRes = await getAttendanceStreak(userRes.data.id);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-3xl shadow-lg p-8">
            <div className="flex flex-col items-center">
              ...
              <div className="flex items-center gap-2 text-gray-600 mb-6">
                ...
                <span className="text-sm">{user.createDate || '-'}</span>
              </div>

              <div className="w-full">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-semibold text-gray-700">Total XP</span>
                  <span className="text-sm font-bold text-blue-500">{user.totalXp || 0} / {user.totalXpGoal || 3000}</span>
                </div>
                <div className="h-3 bg-white rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full"
                    style={{ width: `${(user.totalXp || 0) / (user.totalXpGoal || 3000) * 100}%` }}
                  />
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-3xl shadow-lg p-8">
            ...
            <div className="space-y-3">
              {leaderboard.map((u) => (
                <div
                  key={u.rank}
                  className={`flex items-center justify-between p-4 rounded-2xl transition-all ${u.userId === user.id ? 'bg-blue-50 border-2 border-blue-200' : 'bg-gray-50 hover:bg-gray-100'}`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 bg-gradient-to-br from-gray-400 to-gray-500 rounded-xl flex items-center justify-center text-white font-bold`}>
                      #{u.rank}
                    </div>
                    ...
                    <div>
                      <p className="font-semibold text-gray-900 flex items-center gap-2">
                        {u.fullName}
                        {u.userId === user.id && <span className="text-xs bg-blue-500 text-white px-2 py-0.5 rounded-full">You</span>}
                      </p>
                      <p className="text-sm text-gray-600">{u.email}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-bold text-blue-500">{u.totalScore}</p>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-3xl shadow-lg p-6">
            ...
            <h4 className="text-lg font-bold text-gray-900 mb-1">{latestLesson?.lessonName || '-'}</h4>
            <p className="text-sm text-gray-600 mb-4">{latestLesson?.content || '-'}</p>
            <div className="mb-3">
              <div className="flex items-center justify-between text-sm mb-2">
                <span className="text-gray-600 font-medium">Progress</span>
                <span className="font-bold text-green-500">{latestLesson?.progress || 0}%</span>
              </div>
              <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full bg-green-500 rounded-full" style={{ width: `${latestLesson?.progress || 0}%` }} />
              </div>
            </div>
            <div className="text-sm text-gray-600 bg-green-50 px-3 py-2 rounded-lg">
              <span className="font-semibold text-green-700">Level {latestLesson?.level || 1}</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-lg p-6">
            ...
            <div className="flex justify-center gap-2">
              {[1, 2, 3, 4, 5, 6, 7].map((day, i) => (
                <div key={day} className={`w-8 h-8 rounded-lg flex items-center justify-center ${i < streak ? 'bg-orange-500' : 'bg-gray-200'}`}>
                  {i < streak && <Check className="w-4 h-4 text-white" />}
                </div>
              ))}
            </div>
          </div>
      ...
      {showSettingsModal && (
        <>
          <div className="fixed inset-0 bg-black/50 z-40" onClick={() => setShowSettingsModal(false)} />
          <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl z-50 overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-2xl font-bold text-gray-900">Settings</h2>
              <button onClick={() => setShowSettingsModal(false)} className="w-10 h-10 hover:bg-gray-100 rounded-full flex items-center justify-center transition-colors">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="bg-pink-50 rounded-2xl p-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setTheme('light')} className={`px-6 py-3 rounded-xl font-semibold transition-all ${theme === 'light' ? 'bg-white border-2 border-cyan-400 text-gray-900' : 'bg-white text-gray-700 hover:bg-gray-100'}`}>Light</button>
                  <button onClick={() => setTheme('dark')} className={`px-6 py-3 rounded-xl font-semibold transition-all ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}>Dark</button>
                </div>
              </div>
